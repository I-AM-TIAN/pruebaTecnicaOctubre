...
export class ElevenlabsService {
    ...
    async transcribe(buffer: Buffer, filename = 'audio.ogg') {
        ...
        try {
            ...
            if (error.response) {
                // Es un error de axios con respuesta
                const errorData = error.response?.data;
                const errorStatus = error.response?.status;
                console.error('ElevenLabs API error: ', errorStatus, JSON.stringify(errorData));
                throw new Error(`Transcription failed with status ${errorStatus}: ${JSON.stringify(errorData)}`);
            }
